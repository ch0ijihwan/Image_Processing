...
def print_matInfo(name, image):                         #행렬 정보 출력 함
    ...
    nchannel = 3 if image.ndim == 3 else 1 #RGB 같이 3 채널로 받으면 3이고, 그렇지 않는다면 싱글채널로 1로 함.

    ## depth, channel 출력
    print("%12s: depth(%s), channels(%s) -> mat_type(%sC%d)"
          % (name, image.dtype, nchannel, mat_type,  nchannel))
    # mat_type는 매트릭스 타입으로 위에 함수(print_matInfo)에 따라 지정해줌
...
print("%s %s" % (title1, gray2gray[100, 100])) #변환된 gray2gray 값을 출력해줌
print("%s %s\n" % (title2, gray2color[100, 100]))

print_matInfo(title1, gray2gray)
print_matInfo(title2, gray2color)

cv2.imshow(title1, gray2gray)       #행렬들을 영상으로 바꿔줌
